...
> 要求：插件代码零改动方案 = **独立隧道代理**（推荐 cloudflared，免费、无账号、自动 HTTPS）。隧道进程与 DSH 同机时会把外网请求转成对 `127.0.0.1` 的连接，这与「本机直连」在 DSH 眼里几乎无法区分——所以**必须**同时开启插件侧 `publicMode`，由它收回网关上所有回环免鉴权豁免（详见 `SPEC-v4-外网穿透.md`）。
...
1. **起一条隧道指向网关端口**（红线：**绝不能指 DSH 主端口**，只指网关 `0.0.0.0:<gatewayPort>`，默认 46322）：
   ...
   ```
2. **开启 publicMode**：DSH 桌面 → 设置 → 「DSH Mini 手机桥」→ 开启「外网穿透」。
...
鉴权：回环免 token；非回环 `Authorization: Bearer <token>` / `x-dsh-mini-token` / SSE `?token=`。**外网穿透（publicMode）开启后，网关对一切请求（含隧道转成的回环连接）强制要求 token。**
